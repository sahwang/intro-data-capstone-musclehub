...
df["ab_test_group"] = df.apply(lambda row: "A" if pd.notnull(row["fitness_test_date"]) else "B", axis = 1)
df.head(-1)
...
df["is_application"] = df.apply(lambda row: "Application" if pd.notnull(row["application_date"]) else "No Application", axis = 1)
df.head()
...
import numpy as np
contingency = np.array([[250, 325], [2254, 2175]])
...
just_apps_group = just_apps.groupby(["ab_test_group", "is_member"]).email.count().reset_index()
member_pivot = just_apps_group.pivot(columns = "is_member", index = "ab_test_group", values = "email").reset_index()
...
contingency2 = np.array([[200, 250], [50, 75]])
...
contingency3 = np.array([[200, 250], [2304, 2250]])
...
bar_xlabels = ["Fitness Test", "No Fitness Test"]
plt.bar(range(len(app_counts_pivot)), app_counts_pivot["Percent with Application"])
ax = plt.subplot()
ax.set_xticks([0,1])
ax.set_xticklabels(bar_xlabels)
yvals = ax.get_yticks()
ax.set_yticklabels(['{:,.0%}'.format(x) for x in yvals])
plt.title("Conversion to Application")
...
yvals = ax.get_yticks()
ax.set_yticklabels(['{:,.0%}'.format(x) for x in yvals])
plt.title("Conversion to Membership Purchase (after application)")
plt.ylabel("% Conversion")
plt.show()
...
yvals = ax.get_yticks()
ax.set_yticklabels(['{:,.0%}'.format(x) for x in yvals])
plt.title("Conversion to Membership Purchase")
plt.ylabel("% Conversion")
plt.show()
